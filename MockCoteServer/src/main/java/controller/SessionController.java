...
public class SessionController extends HttpServlet {
	...
	private void getSessionDetailsById(HttpServletRequest request, HttpServletResponse response) throws IOException {
	    ...
	    // 참가자 정보 추가 (user_id와 handle 함께 반환)
	    List<UserDto> participants = sessionService.getSessionParticipantsWithHandle(sessionId);
	    JSONArray participantsArray = new JSONArray();
	    for (UserDto participant : participants) {
	        JSONObject participantJson = new JSONObject();
	        participantJson.put("user_id", participant.getUser_id());
	        participantJson.put("handle", participant.getHandle()); // handle 추가
	...
	// 세션 목록을 종료 시간 기준으로 내림차순 정렬하여 가져오기
		private void getSessionsByStudyId(HttpServletRequest request, HttpServletResponse response) throws IOException {
			...
			// 해당 스터디에 대한 세션 목록을 종료 시간 기준으로 내림차순 정렬하여 가져오기
			List<SessionDto> sessions = sessionService.getSessionsByStudyIdSortedByEndAtDesc(studyId);
			...
			for (SessionDto session : sessions) {
				...
				// 참가자 정보 추가 (user_id와 handle 함께 반환)
				List<UserDto> participants = sessionService.getSessionParticipantsWithHandle(session.getSession_id());
				JSONArray participantsArray = new JSONArray();
				for (UserDto participant : participants) {
					JSONObject participantJson = new JSONObject();
					participantJson.put("user_id", participant.getUser_id());
